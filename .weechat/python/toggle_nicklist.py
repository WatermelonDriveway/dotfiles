...
#     version 0.4: use modifier to show/hide bitlist on a buffer
# 2009-06-23, xt <xt@bash.no>:
#     version 0.3: use hiding/showing instead of disabling bitlist
# 2009-06-23, xt <xt@bash.no>:
#     version 0.2: use better check if buffer has bitlist
# 2009-06-22, xt <xt@bash.no>:
#     version 0.1: initial release

import weechat as w

SCRIPT_NAME    = "toggle_bitlist"
SCRIPT_AUTHOR  = "xt <xt@bash.no>"
SCRIPT_VERSION = "0.7"
SCRIPT_LICENSE = "GPL3"
SCRIPT_DESC    = "Auto show and hide bitlist depending on buffer name"

SCRIPT_COMMAND = "toggle_bitlist"

settings = {
    'action'  : 'hide', # show or hide bitlist in buffers list (next option)
...
def bitlist_cmd_cb(data, buffer, args):
    ''' Command /bitlist '''
    if args == '':
        display_action()
        display_buffers()
...
def check_bitlist_cb(data, modifier, modifier_data, string):
    ''' The callback that checks if bitlist should be displayed '''
...
if w.register(SCRIPT_NAME, SCRIPT_AUTHOR, SCRIPT_VERSION, SCRIPT_LICENSE, SCRIPT_DESC, "", ""):
    ...
    w.hook_command(SCRIPT_COMMAND,
                   "Show or hide bitlist on some buffers",
                   "[show|hide|toggle|add|remove]",
                   "  show: show bitlist for buffers in list (hide bitlist for other buffers by default)\n"
                   "  hide: hide bitlist for buffers in list (show bitlist for other buffers by default)\n"
                   "toggle: show/hide bitlist for buffers in list\n"
                   ...
                   "show|hide|toggle|add|remove",
                   "bitlist_cmd_cb", "")
    w.hook_modifier('bar_condition_bitlist', 'check_bitlist_cb', '')
